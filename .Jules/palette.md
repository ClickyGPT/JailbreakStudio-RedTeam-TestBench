...
## 2025-05-16 - [Input Context in Minimalist UIs]
**Learning:** In minimalist "Studio" UIs, labels are often omitted for aesthetics, leaving inputs with only placeholders. This loses context once text is entered.
**Action:** Always provide explicit `aria-label` and `title` attributes for inputs and icon-only buttons to maintain accessibility and discoverability without compromising the minimalist aesthetic.
