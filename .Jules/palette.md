...
## 2025-05-16 - [Keyboard Accessible CSS Dropdowns]
**Learning:** Pure CSS hover menus (using `group-hover`) are inaccessible to keyboard users. Using Tailwind's `group-focus-within` allows these menus to be toggled by keyboard focus without requiring additional React state.
**Action:** To make CSS-only hover menus accessible, apply `group-focus-within:block` to the dropdown container and add `aria-haspopup="true"` to the trigger button.
