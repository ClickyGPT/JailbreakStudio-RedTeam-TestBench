...
## 2025-05-15 - [Keyboard Accessibility for Dropdowns]
**Learning:** CSS-only hover dropdowns are inaccessible to keyboard users. Using Tailwind's `group-focus-within:block` alongside `group-hover:block` allows menus to remain open when their internal elements receive focus.
**Action:** Always pair `group-hover` with `group-focus-within` for dropdown containers to ensure keyboard navigability.
