...
## 2025-05-16 - [Keyboard Discoverability of Hover Menus]
**Learning:** Using `group-hover:block` for dropdown menus creates a keyboard trap where the menu is unreachable via Tab.
**Action:** Always pair `group-hover:block` with `group-focus-within:block` (or similar focus-based modifiers) and ensure the trigger element has `aria-haspopup="true"` to indicate the submenu existence to screen readers.
