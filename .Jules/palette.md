...
## 2025-05-16 - [Keyboard Accessibility for Hover-Menus]
**Learning:** Tailwind's `group-hover` is excellent for visual effects but leaves keyboard users stranded.
**Action:** Always pair `group-hover` with `group-focus-within` and ensure menu items have `focus-visible` styles to enable full keyboard navigation for dropdowns.
