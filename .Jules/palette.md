...
**Learning:** To improve discoverability of keyboard shortcuts without cluttering the UI, associate them with the `title` attribute of the corresponding action button using platform-specific symbols (e.g., '⌘↵' vs 'Ctrl+Enter').
**Action:** Always use platform detection when displaying keyboard shortcut hints to ensure accuracy for the user's OS and use standard symbols for better visual scanning.
