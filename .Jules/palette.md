...
## 2026-03-17 - [Dropdown Accessibility with focus-within]
**Learning:** Pure CSS dropdowns using `group-hover:block` are inaccessible to keyboard users because hovering is a mouse-only action.
**Action:** Always combine `group-hover:block` with `group-focus-within:block` (and `aria-haspopup="true"` on the trigger) to ensure nested menus are revealed when the parent container or trigger button receives focus via Tab navigation.
