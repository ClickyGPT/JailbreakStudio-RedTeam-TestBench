...
**Learning:** Tailwind's `group-hover` pattern for menus is inaccessible to keyboard users. Using `group-focus-within:block` alongside `group-hover:block` allows the menu to remain visible when its trigger or any child gains focus.
**Action:** Always combine `group-focus-within` with `group-hover` for dropdown menus to ensure they work for both mouse and keyboard users.
