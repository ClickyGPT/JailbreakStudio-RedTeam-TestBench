...
## 2025-07-24 - [Optimizing String Pattern Matching for Refusal Detection]
**Learning:** Iteratively scanning large strings (~100KB) with multiple `toLowerCase().includes()` calls is highly inefficient (O(N*M)) and causes excessive memory allocations.
**Action:** Use a single-pass, pre-compiled `RegExp` with the `i` flag for pattern matching. In this codebase, it provided a ~300x speedup for 100KB output strings.
