...
## 2026-03-18 - [Optimized String Scanning with Pre-compiled Regex]
**Learning:** Iteratively scanning large strings for multiple keywords using `.toLowerCase().includes()` in a hot path causes significant overhead due to repeated string allocations and linear lookups.
**Action:** Use a module-level pre-compiled case-insensitive `RegExp` to perform multi-keyword matching in a single pass, which is significantly faster and more memory-efficient.
