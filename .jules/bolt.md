...
## 2026-04-06 - [Redundant String Transformations in Loops]
**Learning:** Performing `.toLowerCase()` on both the source text and each comparison keyword within a loop (e.g., `.some()` or `.filter()`) results in $2N$ string allocations per check, which significantly impacts performance on large outputs or high-frequency checks.
**Action:** Pre-lowercase constant keyword lists and perform a single-pass `.toLowerCase()` on the source text before entering the matching loop to achieve $O(1)$ allocations instead of $O(N)$.
