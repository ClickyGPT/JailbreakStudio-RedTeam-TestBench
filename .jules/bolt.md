...
## 2025-05-20 - [Redundant String Transformation Bottleneck]
**Learning:** Iteratively calling `.toLowerCase()` on large strings (e.g., 100KB model outputs) within a loop or `.some()` call creates a massive performance bottleneck due to repeated allocations and transformations.
**Action:** Use a pre-compiled, case-insensitive `RegExp` at the module level to perform a single pass over the original string, avoiding both $O(N \cdot M)$ complexity and redundant memory allocations.
