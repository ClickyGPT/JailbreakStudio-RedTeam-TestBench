...
**Learning:** Performing `.toLowerCase()` or similar string transformations inside a loop (e.g., checking multiple keywords against a single source string) leads to unnecessary allocations and CPU churn.
**Action:** Perform a single-pass transformation on the source string before entering the loop and ensure the lookup array (keywords) is pre-transformed to the same format.
