...
// BOLT OPTIMIZATION: Parse URL hash once outside the component to enable lazy initialization
// of state, preventing a redundant second render cycle on mount.
const initialSharedState = decodeStateFromHash();

const App: React.FC = () => {
  const [prompt, setPrompt] = useState<string>(() => initialSharedState?.prompt || '');
  const [result, setResult] = useState<SimulationResult | null>(() => initialSharedState?.lastResult || null);
  ...
  return (
    <div className="flex flex-col h-screen bg-cyber-black text-cyber-text font-sans overflow-hidden relative selection:bg-cyber-lime selection:text-black">
      ...
      {/* BOLT OPTIMIZATION: Isolated spotlight to prevent top-level re-renders from mouse moves */}
      <Spotlight />
