...
const App: React.FC = () => {
  ...
  // BOLT OPTIMIZATION: Use refs to provide stable values for memoized callbacks
  const promptRef = useRef(prompt);
  const resultRef = useRef(result);

  // Sync refs in render body to ensure stable callbacks have access to latest state
  // without triggering re-renders of components that depend on these callbacks.
  ...
  const handleRunTest = React.useCallback(async () => {
    ...
    setResult(simResult);
    setIsRunning(false);
  }, []); // BOLT OPTIMIZATION: Empty dependencies prevent Composer from re-rendering on every keystroke
  ...
  const handleShare = React.useCallback(() => {
    ...
    setShowShare(true);
  }, []); // BOLT OPTIMIZATION: Stable reference prevents unnecessary child re-renders
