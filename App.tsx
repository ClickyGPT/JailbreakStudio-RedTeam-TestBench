...
const App: React.FC = () => {
  ...
  // BOLT OPTIMIZATION: Stabilize callbacks by using refs for state values needed in closures
  const promptRef = useRef(prompt);
  const resultRef = useRef(result);

  // BOLT OPTIMIZATION: Sync refs in effect to maintain pure render
  ...
  const handleRunTest = React.useCallback(async () => {
    ...
    setResult(simResult);
    setIsRunning(false);
  }, []); // BOLT OPTIMIZATION: Removed prompt dependency to prevent callback recreation
  ...
  const handleShare = React.useCallback(() => {
    ...
    setShowShare(true);
  }, []); // BOLT OPTIMIZATION: Removed prompt/result dependencies
