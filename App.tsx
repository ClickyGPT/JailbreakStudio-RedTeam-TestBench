...
// BOLT OPTIMIZATION: Parse initial state from hash once outside the component
// to avoid redundant renders and effect-based state initialization.
const sharedState = decodeStateFromHash();

const App: React.FC = () => {
  const [prompt, setPrompt] = useState<string>(() => sharedState?.prompt || '');
  const [result, setResult] = useState<SimulationResult | null>(() => sharedState?.lastResult || null);
  ...
  return (
    <div className="flex flex-col h-screen bg-cyber-black text-cyber-text font-sans overflow-hidden relative selection:bg-cyber-lime selection:text-black">
      ...
      {/* BOLT OPTIMIZATION: Memoized spotlight component isolates high-frequency mouse tracking */}
      <Spotlight />
