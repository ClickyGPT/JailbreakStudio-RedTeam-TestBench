    ...
    <div className="flex-1 flex flex-col h-full relative">
      ...
      <div className="p-4 border-t border-gray-900 bg-cyber-black flex justify-between items-center">
        ...
        <button
            ...
            title={`Initiate Test (${isMac ? '⌘' : 'Ctrl+'}↵)`}
        >
            <span className="relative z-10 flex items-center gap-2">
                <Play size={16} fill="currentColor" />
                {isRunning ? 'SIMULATING...' : 'INITIATE TEST'}
                {!isRunning && (
                  <span id="shortcut-hint" className="ml-1 text-[10px] opacity-50 font-mono hidden group-hover:inline group-focus-visible:inline transition-opacity">
                    ({isMac ? '⌘' : 'Ctrl+'}↵)
                  </span>
                )}
            </span>
