  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      <div className="p-3 border-b border-gray-900 flex justify-between items-center bg-cyber-black">
        ...
        <div className="flex gap-2">
             ...
             <div className="relative group/augment">
                <button 
                    aria-haspopup="true"
                    className="px-3 py-1 text-xs font-mono font-bold bg-cyber-lime/5 text-cyber-lime border border-cyber-lime/30 rounded hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all uppercase tracking-wider outline-none"
                    disabled={isAugmenting}
                    title="Use AI to rewrite and enhance your attack vector"
                >
                   <Wand2 size={12} /> {isAugmenting ? 'PROCESSING...' : 'AI AUGMENT'}
                </button>
                <div className="absolute right-0 top-full mt-1 w-48 bg-cyber-black border border-gray-800 rounded shadow-[0_0_20px_rgba(0,0,0,0.8)] hidden group-hover/augment:block group-focus-within/augment:block z-20 backdrop-blur-xl">
                    <button 
                        onClick={() => handleAugment('obfuscate')} 
                        className="block w-full text-left px-4 py-3 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black text-gray-300 transition-colors border-b border-gray-900 outline-none"
                    ...
                    <button 
                        ...
                        className="block w-full text-left px-4 py-3 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black text-gray-300 transition-colors outline-none"
             ...
             <div className="relative group/vars">
                <button 
                    aria-haspopup="true"
                    className="px-3 py-1 text-xs font-mono font-bold bg-gray-900 text-cyber-muted border border-gray-700 rounded hover:border-cyber-lime hover:text-cyber-lime focus-visible:border-cyber-lime focus-visible:text-cyber-lime transition-all uppercase tracking-wider outline-none"
                    title="Insert predefined or custom prompt variables"
                >
                   + VARS
                </button>
                <div className="absolute right-0 top-full mt-1 w-64 bg-cyber-black border border-gray-800 rounded shadow-2xl hidden group-hover/vars:block group-focus-within/vars:block z-20 max-h-80 overflow-y-auto backdrop-blur-xl">
                    {systemVars.length > 0 && (
                    ...
                    {systemVars.map((v) => (
                        <button 
                            ...
                            className="block w-full text-left px-4 py-2 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black text-gray-300 border-b border-gray-900 last:border-0 group/item transition-all outline-none"
                    ...
                    <div className="border-t border-gray-800 sticky bottom-0 bg-cyber-black">
                        <button 
                            ...
                            className="block w-full text-left px-4 py-3 text-xs bg-gray-900 hover:bg-gray-800 text-cyber-lime focus-visible:bg-gray-800 font-bold flex items-center gap-2 transition-colors uppercase outline-none"
      ...
      <div className="flex-1 relative bg-[#0a0a0a] overflow-hidden group">
        ...
        <div className="absolute bottom-4 right-4 text-[10px] font-mono text-gray-600 pointer-events-none select-none">
            {prompt.length} CHARS
        </div>
      </div>
