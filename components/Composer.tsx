    ...
    <div className="flex-1 flex flex-col h-full relative">
      <div className="p-3 border-b border-gray-900 flex justify-between items-center bg-cyber-black">
        <div className="flex items-center gap-2">
            <Terminal size={16} className="text-cyber-muted" aria-hidden="true" />
        ...
        <div className="flex gap-2">
             ...
             <div className="relative group">
                <button 
                    ...
                    aria-haspopup="true"
                >
                   <Wand2 size={12} aria-hidden="true" /> {isAugmenting ? 'PROCESSING...' : 'AI AUGMENT'}
                </button>
                ...
                <div className="absolute right-0 top-full mt-1 w-64 bg-cyber-black border border-gray-800 rounded shadow-2xl hidden group-hover:block group-focus-within:block z-20 max-h-80 overflow-y-auto backdrop-blur-xl">
                    {systemVars.length > 0 && (
                        ...
                        <span className="text-[10px] font-bold text-cyber-muted uppercase flex items-center gap-1 tracking-widest">
                            <Sparkles size={10} aria-hidden="true" /> Core Variables
                        </span>
                        </div>
                    )}
                    {systemVars.map((v) => (
                    ...
                    {customVars.length > 0 && (
                        ...
                        <span className="text-[10px] font-bold text-cyber-muted uppercase flex items-center gap-1 tracking-widest">
                            <Box size={10} aria-hidden="true" /> Custom Variables
                        </span>
                        </div>
                    )}
                    {customVars.map((v) => (
                    ...
                    <div className="border-t border-gray-800 sticky bottom-0 bg-cyber-black">
                        <button 
                            ...
                            className="block w-full text-left px-4 py-3 text-xs bg-gray-900 hover:bg-gray-800 text-cyber-lime font-bold flex items-center gap-2 transition-colors uppercase outline-none focus-visible:bg-gray-800"
                            title="Add, edit, or remove custom variables"
                        >
                            <Settings size={12} aria-hidden="true" /> MANAGE VARIABLES
                        </button>
                    </div>
                </div>
             </div>
      ...
      <div className="flex-1 relative bg-[#0a0a0a] overflow-hidden group">
        ...
        <div className="absolute bottom-4 right-6 pointer-events-none select-none text-[10px] font-mono text-gray-700 uppercase tracking-widest flex items-center gap-2">
            <span className={prompt.length > 0 ? 'text-cyber-lime/40' : ''}>{prompt.length} CHARS</span>
            <span className="text-gray-800">|</span>
            <span className="text-gray-800">CTRL+ENTER TO RUN</span>
        </div>
      ...
      <div className="p-4 border-t border-gray-900 bg-cyber-black flex justify-between items-center">
        <div className="flex items-center gap-4">
            <button
                onClick={onShare}
                className="flex items-center gap-2 text-xs font-bold font-sans text-cyber-muted hover:text-white transition-colors uppercase tracking-widest outline-none focus-visible:text-white"
                title="Generate a unique link to share this prompt vector"
            >
                <Share2 size={14} aria-hidden="true" /> SHARE VECTOR
            </button>
            <button
                onClick={handleCopy}
                disabled={!prompt.trim()}
                className="flex items-center gap-2 text-xs font-bold font-sans text-cyber-muted hover:text-white transition-colors uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:text-white"
                title="Copy prompt to clipboard"
            >
                {isCopied ? <Check size={14} className="text-cyber-lime" aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}
        ...
        <button
            ...
            className={`flex items-center gap-2 px-8 py-3 text-sm font-bold font-sans tracking-wider transition-all duration-300 relative overflow-hidden group outline-none focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:ring-offset-2 focus-visible:ring-offset-black ${
                isRunning 
