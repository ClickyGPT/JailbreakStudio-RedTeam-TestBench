...
const Composer: React.FC<ComposerProps> = React.memo(({ prompt, setPrompt, onRunTest, isRunning, onShare }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      <div className="p-3 border-b border-gray-900 flex justify-between items-center bg-cyber-black">
        ...
        <div className="flex gap-2">
             ...
             <button
                ...
                className="px-3 py-1 text-xs font-mono font-bold bg-cyber-lime/5 text-cyber-lime border border-cyber-lime/30 rounded hover:bg-cyber-lime hover:text-black focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:ring-offset-2 focus-visible:ring-offset-black outline-none flex items-center gap-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider"
             ...
             <div className="relative group">
                <button 
                    className="px-3 py-1 text-xs font-mono font-bold bg-cyber-lime/5 text-cyber-lime border border-cyber-lime/30 rounded hover:bg-cyber-lime hover:text-black focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:ring-offset-2 focus-visible:ring-offset-black outline-none flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all uppercase tracking-wider"
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-cyber-black border border-gray-800 rounded shadow-[0_0_20px_rgba(0,0,0,0.8)] hidden group-hover:block group-focus-within:block z-20 backdrop-blur-xl">
                    <button 
                        ...
                        className="block w-full text-left px-4 py-3 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black outline-none text-gray-300 transition-colors border-b border-gray-900"
                    ...
                    <button 
                        ...
                        className="block w-full text-left px-4 py-3 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black outline-none text-gray-300 transition-colors"
             ...
             <div className="relative group">
                <button 
                    className="px-3 py-1 text-xs font-mono font-bold bg-gray-900 text-cyber-muted border border-gray-700 rounded hover:border-cyber-lime hover:text-cyber-lime focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:ring-offset-2 focus-visible:ring-offset-black outline-none transition-all uppercase tracking-wider"
                    aria-haspopup="true"
                ...
                <div className="absolute right-0 top-full mt-1 w-64 bg-cyber-black border border-gray-800 rounded shadow-2xl hidden group-hover:block group-focus-within:block z-20 max-h-80 overflow-y-auto backdrop-blur-xl">
                    ...
                    {systemVars.map((v) => (
                        <button 
                            ...
                            className="block w-full text-left px-4 py-2 text-xs hover:bg-cyber-lime hover:text-black focus-visible:bg-cyber-lime focus-visible:text-black outline-none text-gray-300 border-b border-gray-900 last:border-0 group/item transition-all"
                            title={`Insert: ${v.value}`}
                        >
                            <div className="flex justify-between items-center">
                                <span className="text-cyber-lime font-bold group-hover/item:text-black group-focus-visible/item:text-black transition-colors">{v.name}</span>
                            </div>
                            <span className="text-[10px] text-gray-500 truncate block group-hover/item:text-gray-800 group-focus-visible/item:text-gray-800">{v.value}</span>
                        </button>
                    ))}
                    ...
                    {customVars.map((v) => (
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <span className="text-white font-bold group-hover/item:text-black group-focus-visible/item:text-black transition-colors">{v.name}</span>
                            </div>
                            <span className="text-[10px] text-gray-500 truncate block group-hover/item:text-gray-800 group-focus-visible/item:text-gray-800">{v.value}</span>
                        </button>
                    ))}
                    ...
                    <div className="border-t border-gray-800 sticky bottom-0 bg-cyber-black">
                        <button 
                            ...
                            className="block w-full text-left px-4 py-3 text-xs bg-gray-900 hover:bg-gray-800 focus-visible:bg-cyber-lime focus-visible:text-black outline-none text-cyber-lime font-bold flex items-center gap-2 transition-colors uppercase"
      ...
      <div className="flex-1 relative bg-[#0a0a0a] overflow-hidden group focus-within:ring-1 focus-within:ring-cyber-lime/30 transition-all">
        <textarea
            ref={setTextAreaRef}
            value={prompt}
            ...
            className="w-full h-full bg-transparent text-gray-200 font-mono p-6 resize-none focus:outline-none text-sm leading-relaxed placeholder-gray-800 selection:bg-cyber-lime selection:text-black"
            spellCheck={false}
        />
      ...
      <div className="p-4 border-t border-gray-900 bg-cyber-black flex justify-between items-center">
        <div className="flex items-center gap-4">
            <button
                onClick={onShare}
                className="flex items-center gap-2 text-xs font-bold font-sans text-cyber-muted hover:text-white focus-visible:text-cyber-lime focus-visible:outline-none transition-colors uppercase tracking-widest"
            ...
            <button
                ...
                className="flex items-center gap-2 text-xs font-bold font-sans text-cyber-muted hover:text-white focus-visible:text-cyber-lime focus-visible:outline-none transition-colors uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
            ...
            className={`flex items-center gap-2 px-8 py-3 text-sm font-bold font-sans tracking-wider transition-all duration-300 relative overflow-hidden group focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:outline-none ${
                isRunning 
