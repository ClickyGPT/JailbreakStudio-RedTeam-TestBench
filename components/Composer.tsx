  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
             ...
             <div className="relative group">
                ...
                <div className="absolute right-0 top-full mt-1 w-64 bg-cyber-black border border-gray-800 rounded shadow-2xl hidden group-hover:block group-focus-within:block z-20 max-h-80 overflow-y-auto backdrop-blur-xl">
                    ...
                    {systemVars.map((v) => (
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <span className="text-cyber-lime font-bold group-hover/item:text-black group-focus-within/item:text-black transition-colors">{v.name}</span>
                            </div>
                            <span className="text-[10px] text-gray-500 truncate block group-hover/item:text-gray-800 group-focus-within/item:text-gray-800">{v.value}</span>
                        </button>
                    ))}
                    ...
                    {customVars.map((v) => (
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <span className="text-white font-bold group-hover/item:text-black group-focus-within/item:text-black transition-colors">{v.name}</span>
                            </div>
                            <span className="text-[10px] text-gray-500 truncate block group-hover/item:text-gray-800 group-focus-within/item:text-gray-800">{v.value}</span>
                        </button>
                    ))}
      ...
      <div className="flex-1 relative bg-[#0a0a0a] overflow-hidden group">
        ...
        <div className="absolute bottom-4 right-6 pointer-events-none select-none flex items-center gap-2">
            <div className="h-px w-8 bg-gray-800"></div>
            <span className="text-[10px] font-mono text-gray-700 tabular-nums tracking-widest">
                LEN: {prompt.length.toLocaleString()}
            </span>
        </div>
