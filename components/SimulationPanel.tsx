...
const SimulationPanel: React.FC<SimulationPanelProps> = React.memo(({ result, isRunning }) => {
  ...
  // BOLT OPTIMIZATION: Using result.prompt (from the time of test) instead of live currentPrompt
  // prevents the entire Results panel from re-rendering while the user is typing in the composer.
        ...
        <div>
            ...
            <div className="p-5 bg-black border border-gray-800 text-xs text-gray-400 space-y-3 font-mono">
                ...
                <div className="flex justify-between">
                    ...
                    <span>{result?.latency !== undefined ? `~${Math.round(result.latency)}ms` : 'N/A'}</span>
                </div>
            </div>
        </div>
