...
const SimulationPanel: React.FC<SimulationPanelProps> = React.memo(({ result, isRunning }) => {
  ...
  const handleAnalyze = async () => {
    if (!result) return;
    setIsAnalyzing(true);
    // BOLT OPTIMIZATION: Use prompt from result object instead of current live state
    // This allows the component to be decoupled from the Composer's typing state
