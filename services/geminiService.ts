...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Perform a single-pass toLowerCase() on the output string
    // and use pre-lowercased keywords to avoid redundant transformations in the loop.
