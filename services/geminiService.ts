...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  // BOLT OPTIMIZATION: Using performance.now() for millisecond-precision timing.
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Lowercase the source string once before the loop and use
    // pre-lowercased keywords to avoid redundant O(N*M) transformations.
