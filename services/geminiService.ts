...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Single-pass lowercasing for the isRefusal check
    // instead of lowercasing the output text for every keyword check.
    const lowerOutputText = outputText.toLowerCase();
    const isRefusal = REFUSAL_KEYWORDS.some(keyword => 
      lowerOutputText.includes(keyword)
    );
