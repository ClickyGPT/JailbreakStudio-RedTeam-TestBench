...
// This is significantly faster than iteratively checking keywords using .some() and .includes(),
// especially for long model outputs, as it avoids multiple passes and repeated string allocations.
...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Use pre-compiled regex for O(n) refusal check instead of O(n*m) iterative check.
