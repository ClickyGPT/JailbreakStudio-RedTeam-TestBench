...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Format high-precision latency to 2 decimal places
    ...
    // BOLT OPTIMIZATION: Single toLowerCase() on output, avoids redundant transformations in the loop
    const normalizedOutput = outputText.toLowerCase();
    const isRefusal = REFUSAL_KEYWORDS.some(keyword => normalizedOutput.includes(keyword));
