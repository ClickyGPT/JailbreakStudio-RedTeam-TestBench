 ...
 * Using a single case-insensitive RegExp is significantly faster than iteratively
 * calling .toLowerCase().includes() on large strings, as it avoids repeated
 * string transformations and multiple passes over the input.
 */
...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Use pre-compiled regex for O(n) scan instead of O(n*m) iterative search
