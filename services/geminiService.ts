...
// This is significantly faster (~3.5x in worst-case scenarios) than iterative
// .toLowerCase().includes() calls on large strings, as it avoids redundant
// string transformations and multiple passes.
const REFUSAL_REGEX = new RegExp(
  REFUSAL_KEYWORDS.length > 0
    ? REFUSAL_KEYWORDS.map(k => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')
    : '$.^', // Regex that never matches (fallback for empty keyword list)
  'i'
);
