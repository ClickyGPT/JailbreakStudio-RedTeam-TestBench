...
export const simulateAttack = async (prompt: string): Promise<SimulationResult> => {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Ensure consistent high-precision decimal formatting
    ...
    // BOLT OPTIMIZATION: Perform single-pass toLowerCase on the output text
    // and match against pre-lowercased keywords for ~9x faster refusal detection.
